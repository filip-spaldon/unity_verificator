...
func forParserLeft(input string, variables map[string]interface{}) (int, string, int, string, string) {
	...
	if strings.Contains(input, "<<||") && strings.Contains(input, ">>") {
		input = input[strings.Index(input, "<<||")+4 : strings.Index(input, ">>")]
...
func forParserRight(input string, variables map[string]interface{}, i int, k string, N int, oper1 string, oper2 string) ([][]int, []string) {
	var array []int
	arrays := make([][]int, 0)
	vals := make([]string, 0)
	if strings.Contains(input, "<<||") && strings.Contains(input, ">>") {
				...
				if err1 == nil && err2 == nil {
					...
					if err1 == nil && err2 == nil {
						if result1.(bool) && result2.(bool) {
							if strings.Contains(strings.Split(_right, ",")[i], "rand") {
								array[index] = rand.Intn(100)
							}
							index++
						} else {
							break
		...
		if str, ok := val.(string); ok {
			if strings.Contains(str, "if") {
				...
				if len(strings.Split(r, " ")) == 1 {
					...
					pom.Nodes = append(pom.Nodes, &Node{Statement: "=", Ref: Tree.Find(ref).Nodes[0]})
				} else {
					for i, val := range strings.Split(r, " ") {
						if exp := makeExp(str, i, val, []string{">", "<", ">=", "<=", "==", "!="}); exp != nil {
							pom.Nodes = append(pom.Nodes, &Node{Statement: "=", Ref: exp})
						}
					}
				}
			} else {
				pom.Nodes = append(pom.Nodes, &Node{Statement: "="})
			}
...
// 		statement: "==",
// 	}
// 	node.nodes = append(node.nodes, &Node{value: 10, name: "a"})
// 	fmt.Println(node)
// }
